...
function summariseByBrand(items, monthsActive){
  ...
  for(const d of items){
    ...
    for (const m of monthsActive){
      ...
      const u   = d.months[m];
      b.revenue += Number.isFinite(rev)? rev : 0;
      b.units   += Number.isFinite(u)?   u   : 0;
    }
  }
...
function drawBrandMonthlyStacked(items){
  ...
  const traces = rows.map((r,i)=>({
    ...
    name:r.brand,
    x:months,
    y: months.map(m => sum(items.filter(d=>d.brand===r.brand).map(d => Number.isFinite(d.months[m])? d.months[m] : 0))),
